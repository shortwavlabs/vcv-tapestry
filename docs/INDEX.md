# Tapestry Documentation Index

**Complete documentation for the Tapestry VCV Rack plugin**

---

## 📚 Documentation Structure

### Main Documentation

- **[README.md](../README.md)** - Main overview, features, installation, and quick reference
- **[Quick Start Guide](QUICKSTART.md)** - Get up and running in minutes
- **[Korupt User Manual](KORUPT.md)** - PLL harmonizer/fuzz controls, patching, presets, and tracking tips
- **[Wyrd User Manual](WYRD.md)** - Tone core, external processing, feedback, touch bridges, reverb, and patch recipes
- **[Drift User Manual](DRIFT.md)** - Fluctuating random CV, Signal B blending, voltage ranges, and patch recipes
- **[API Reference](API_REFERENCE.md)** - Complete technical documentation
- **[Advanced Usage](ADVANCED_USAGE.md)** - Expert techniques and workflows
- **[FAQ](FAQ.md)** - Common questions and solutions

### Project Information

- **[Changelog](CHANGELOG.md)** - Version history and updates
- **[Contributing Guidelines](CONTRIBUTING.md)** - How to contribute to the project
- **[Terminology Mapping](TERMINOLOGY_MAPPING.md)** - Legal compliance documentation

### Practical Examples

- **[Examples Directory](examples/)** - Real-world implementation samples
  - [Basic Recording](examples/01_basic_recording.md)
  - [Granular Textures](examples/02_granular_textures.md)
  - [Rhythmic Processing](examples/03_rhythmic_processing.md)
  - [Expander Effects](examples/07_expander_effects.md)

---

## 🎯 Quick Navigation by Task

### I want to...

#### Get Started
→ [Quick Start Guide](QUICKSTART.md)  
→ [Example 1: Basic Recording](examples/01_basic_recording.md)

#### Learn the Basics
→ [Quick Start Guide](QUICKSTART.md)  
→ [FAQ](FAQ.md)

#### Create Ambient Textures
→ [Example 2: Granular Textures](examples/02_granular_textures.md)  
→ [Advanced Usage: Granular Techniques](ADVANCED_USAGE.md#advanced-granular-techniques)

#### Process Rhythmic Material
→ [Example 3: Rhythmic Processing](examples/03_rhythmic_processing.md)  
→ [Advanced Usage: Clock Synchronization](ADVANCED_USAGE.md#clock-synchronization-mastery)

#### Use the Expander
→ [Example 7: Expander Effects](examples/07_expander_effects.md)  
→ [Quick Start: Adding Effects](QUICKSTART.md#adding-effects-with-expander)

#### Use Korupt
→ [Korupt User Manual](KORUPT.md)  
→ [Korupt Presets](KORUPT.md#factory-presets)

#### Use Wyrd
- [Wyrd User Manual](WYRD.md)
- [Wyrd First Patch](WYRD.md#first-patch)
- [Wyrd Factory Presets](WYRD.md#factory-presets)

#### Use Drift
- [Drift User Manual](DRIFT.md)
- [Drift First Patch](DRIFT.md#first-patch)
- [Drift Patch Recipes](DRIFT.md#patch-recipes)

#### Understand the Technical Details
→ [API Reference](API_REFERENCE.md)  
→ [Advanced Usage](ADVANCED_USAGE.md)

#### Optimize Performance
→ [Advanced Usage: Performance Optimization](ADVANCED_USAGE.md#performance-optimization)  
→ [FAQ: Performance and CPU](FAQ.md#performance-and-cpu)

#### Contribute to the Project
→ [Contributing Guidelines](CONTRIBUTING.md)  
→ [Changelog](CHANGELOG.md)

#### Troubleshoot Issues
→ [FAQ: Troubleshooting](FAQ.md#troubleshooting)  
→ [Quick Start: Troubleshooting](QUICKSTART.md#troubleshooting)

---

## 📖 Learning Path

### Beginner
1. Read [Quick Start Guide](QUICKSTART.md)
2. Follow [Example 1: Basic Recording](examples/01_basic_recording.md)
3. Try [Example 2: Granular Textures](examples/02_granular_textures.md)
4. Explore [FAQ](FAQ.md) as questions arise

### Intermediate
1. Study [Example 3: Rhythmic Processing](examples/03_rhythmic_processing.md)
2. Experiment with [Example 7: Expander Effects](examples/07_expander_effects.md)
3. Review [API Reference](API_REFERENCE.md) for parameter details
4. Read [Advanced Usage](ADVANCED_USAGE.md) techniques

### Advanced
1. Master [Advanced Usage](ADVANCED_USAGE.md) workflows
2. Study [API Reference](API_REFERENCE.md) DSP details
3. Review [Contributing Guidelines](CONTRIBUTING.md)
4. Create custom patches and share with community

---

## 🔍 Documentation by Topic

### Recording
- [Quick Start: Basic Recording](QUICKSTART.md#basic-recording)
- [Example 1: Basic Recording](examples/01_basic_recording.md)
- [API: Recording Methods](API_REFERENCE.md#recording-and-playback)
- [FAQ: Recording Questions](FAQ.md#recording-and-playback)

### Splices
- [Quick Start: Creating Splices](QUICKSTART.md#creating-splices)
- [Advanced: Marker Management](ADVANCED_USAGE.md#splice-management-strategies)
- [API: SpliceManager Class](API_REFERENCE.md#class-splicemanager)
- [FAQ: Splices](FAQ.md#splices-and-navigation)

### Granular Processing
- [Quick Start: Granular Processing](QUICKSTART.md#granular-processing)
- [Example 2: Granular Textures](examples/02_granular_textures.md)
- [Advanced: Granular Techniques](ADVANCED_USAGE.md#advanced-granular-techniques)
- [API: GrainEngine Class](API_REFERENCE.md#class-grainengine)

### Clock Synchronization
- [Quick Start: Time Stretch Mode](QUICKSTART.md#time-stretch-mode)
- [Example 3: Rhythmic Processing](examples/03_rhythmic_processing.md)
- [Advanced: Clock Sync Mastery](ADVANCED_USAGE.md#clock-synchronization-mastery)
- [FAQ: Clock Sync](FAQ.md#clock-synchronization)

### Expander Effects
- [Quick Start: Adding Effects](QUICKSTART.md#adding-effects-with-expander)
- [Example 7: Expander Effects](examples/07_expander_effects.md)
- [API: TapestryExpander Class](API_REFERENCE.md#tapestry-expander-module)
- [FAQ: Expander](FAQ.md#expander-module)

### Korupt PLL Harmonizer
- [Korupt User Manual](KORUPT.md)
- [Korupt Factory Presets](KORUPT.md#factory-presets)
- [Korupt Tracking Tips](KORUPT.md#tracking-tips)

### Wyrd Tone and Feedback
- [Wyrd User Manual](WYRD.md)
- [Wyrd Signal Flow](WYRD.md#signal-flow)
- [Wyrd Touch Bridges](WYRD.md#touch-bridges)
- [Wyrd Factory Presets](WYRD.md#factory-presets)
- [Wyrd Troubleshooting](WYRD.md#troubleshooting)

### Drift Random CV
- [Drift User Manual](DRIFT.md)
- [Drift Controls](DRIFT.md#controls)
- [Drift Patch Recipes](DRIFT.md#patch-recipes)
- [Drift Troubleshooting](DRIFT.md#troubleshooting)

### CV Modulation
- [Quick Start: CV Inputs](QUICKSTART.md#cv-voltage-standards)
- [Advanced: CV Modulation Recipes](ADVANCED_USAGE.md#cv-modulation-recipes)
- [API: CV Input Methods](API_REFERENCE.md#cv-input-methods)

### Performance
- [Advanced: Performance Optimization](ADVANCED_USAGE.md#performance-optimization)
- [FAQ: Performance and CPU](FAQ.md#performance-and-cpu)
- [API: Performance Considerations](API_REFERENCE.md#performance-considerations)

---

## 📝 Document Summaries

### README.md
**Purpose**: Project overview and quick reference  
**Audience**: All users  
**Length**: ~500 lines  
**Topics**: Features, installation, basic usage, quick reference tables

### QUICKSTART.md
**Purpose**: Get started quickly with hands-on tutorials  
**Audience**: Beginners  
**Length**: ~600 lines  
**Topics**: Step-by-step guides, basic workflows, troubleshooting

### KORUPT.md
**Purpose**: User manual for the Korupt PLL harmonizer/fuzz module  
**Audience**: All users  
**Length**: ~400 lines  
**Topics**: Controls, inputs, outputs, presets, patch recipes, tracking, troubleshooting

### WYRD.md
**Purpose**: User manual for the Wyrd tone source and nonlinear feedback processor

**Audience**: All users

**Length**: ~450 lines

**Topics**: Signal flow, controls, CV, touch bridges, reverb, presets, patch recipes, troubleshooting

### DRIFT.md
**Purpose**: User manual for the Drift fluctuating random LFO

**Audience**: All users

**Length**: ~250 lines

**Topics**: Controls, Signal B blending, voltage ranges, patch recipes, troubleshooting

### API_REFERENCE.md
**Purpose**: Complete technical reference  
**Audience**: Advanced users, developers  
**Length**: ~800 lines  
**Topics**: All classes, methods, parameters, data structures, examples

### ADVANCED_USAGE.md
**Purpose**: Expert techniques and workflows  
**Audience**: Intermediate to advanced users  
**Length**: ~700 lines  
**Topics**: Advanced techniques, optimization, creative workflows, integration

### FAQ.md
**Purpose**: Common questions and solutions  
**Audience**: All users  
**Length**: ~500 lines  
**Topics**: Troubleshooting, how-to questions, explanations

### CHANGELOG.md
**Purpose**: Version history and migration guides  
**Audience**: All users, developers  
**Length**: ~300 lines  
**Topics**: Version changes, new features, bug fixes, migration

### CONTRIBUTING.md
**Purpose**: Guidelines for contributors  
**Audience**: Contributors, developers  
**Length**: ~600 lines  
**Topics**: Code standards, submission process, development setup

---

## 🎓 Recommended Reading Order

### First Time Users
1. [README.md](../README.md) - Overview (5 min)
2. [Quick Start Guide](QUICKSTART.md) - Basics (20 min)
3. [Example 1](examples/01_basic_recording.md) - Practice (10 min)
4. [FAQ](FAQ.md) - Reference (as needed)

### Creative Users
1. [Quick Start Guide](QUICKSTART.md) - Review basics
2. [Example 2: Granular Textures](examples/02_granular_textures.md)
3. [Example 3: Rhythmic Processing](examples/03_rhythmic_processing.md)
4. [Wyrd User Manual](WYRD.md) - Feedback instrument and sound design
5. [Drift User Manual](DRIFT.md) - Organic CV movement
6. [Advanced Usage](ADVANCED_USAGE.md) - Techniques

### Technical Users
1. [API Reference](API_REFERENCE.md) - Technical details
2. [Advanced Usage](ADVANCED_USAGE.md) - Deep dive
3. [Contributing](CONTRIBUTING.md) - Development info
4. [Changelog](CHANGELOG.md) - Version history

---

## 📊 Documentation Statistics

- **Total Pages**: 13 major documents
- **Total Length**: ~5,600 lines
- **Examples**: 4+ detailed tutorials
- **Code Samples**: 50+ snippets
- **Reference Tables**: 20+ tables
- **Topics Covered**: 100+ individual topics

---

## 🔗 External Resources

### VCV Rack
- [VCV Rack Manual](https://vcvrack.com/manual/)
- [VCV Rack SDK](https://vcvrack.com/manual/PluginDevelopmentTutorial)
- [VCV Community](https://community.vcvrack.com/)

### Granular Synthesis
- [Wikipedia: Granular Synthesis](https://en.wikipedia.org/wiki/Granular_synthesis)
- Curtis Roads - "Microsound" (book)
- Barry Truax - Granular Synthesis resources

### Musique Concrète
- [Wikipedia: Musique Concrète](https://en.wikipedia.org/wiki/Musique_concrète)
- Pierre Schaeffer - "Treatise on Musical Objects"
- GRM (Groupe de Recherches Musicales) resources

---

## 💡 Tips for Using This Documentation

1. **Use the search function** in your editor/browser (Ctrl/Cmd + F)
2. **Bookmark frequently used pages**
3. **Follow the examples** hands-on in VCV Rack
4. **Refer to the FAQ** when stuck
5. **Check the API Reference** for technical details
6. **Read the Advanced Usage** for inspiration
7. **Contribute** improvements you discover!

---

## 📧 Contact and Support

- **Issues**: [GitHub Issues](https://github.com/shortwavlabs/vcv-tapestry/issues)
- **Email**: contact@shortwavlabs.com
- **Website**: [shortwavlabs.com](https://shortwavlabs.com)

---

## 📜 License

All documentation is part of the Tapestry project and is licensed under **GPL-3.0-or-later**.

See [LICENSE](../LICENSE.md) for details.

---

**Happy patching! 🎵**

*Last Updated: August 7, 2026*
